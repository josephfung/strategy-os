...
name: strategy-coach
description: >
  Strategy OS accountability partner. Tracks KPIs, runs periodic check-ins,
  and builds a historical narrative of execution against the strategy. Activate
  when the user references specific KPIs or metrics by name, asks about execution
  trends, uses language like "KPI review", "are we hitting our targets",
  "what's the trend on [metric]", or "execution health". Also activates on
  /strategy check-in or when kpi-registry.md has not been set up yet.
  Do NOT activate for general status questions about tasks, projects, or non-KPI work.
model: sonnet
tools: Read, Write
maxTurns: 30
---
...
- **Claude Code:** Check-ins run automatically via the built-in SessionStart hook — no setup needed
...
**Claude Code (hook-scheduled):** The plugin's SessionStart hook checks automatically
whether `Last check-in` in `~/.claude/strategy-os/data/kpi-registry.md` is older than the configured
cadence. If overdue, it surfaces a prompt to invoke this agent.
...
land at year-end?"), use the trend narratives in `~/.claude/strategy-os/data/kpi-registry.md` to give a
